      ...
      {moves.map(move => (
        <Box key={move.estimate_id} p="5" shadow="md" borderWidth="1px" mb="4">
          <Flex alignItems="center" mb="2">
            ...
            <Text fontWeight="bold" fontSize="sm" ml={8}>Request#: </Text>
            <Text color={'tomato'} ml={2}>{move.estimate_id}</Text>
          </Flex>
          <Flex mb="2">
            <Icon as={IoMdHome} ml="2" mr="2" size={24} color={'tomato'} />
            <Text>{move.property_size}</Text>
            <Icon as={GiPathDistance} ml="6" mr="2" size={24} color={'tomato'} />
          ...
          <Flex mb="4">
            <Button color={'tomato'} borderColor="tomato" variant="outline" onClick={() => handleToggle(move.estimate_id)}>
              {expandedMove === move.estimate_id ? 'Hide Details' : 'View move details'}
            </Button>
            <Button backgroundColor={'tomato'} color={'white'} ml="2">Quotes Awaiting</Button>
          </Flex>
